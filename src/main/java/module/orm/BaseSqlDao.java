 ...
 */
public abstract class BaseSqlDao <M extends Serializable, PK extends Serializable> {
    ...
    public BaseSqlDao (Class <M> c, String idName) {
    ...
    public Session getCurrentSession () {
        ...
        return sessionFactorySpot.getCurrentSession ();
    }
    
    protected void setSessionFactory (SessionFactory session) {

        this.sessionFactorySpot = session;
    }
    ...
    @SuppressWarnings("unchecked")
    protected <T> List <T> list (final String hql, final int pageNumber, final int pageSize, final Object ... paramList) {
    ...
    @SuppressWarnings("unchecked")
    public <T> List <T> query (final String sql, final List <Object> obj) {

        SQLQuery query = getCurrentSession ().createSQLQuery (sql);
        for (int i = 0; i < obj.size (); i++) {
            query.setParameter (i, obj.get (i));
        }
        return (List <T>) query.addEntity (this.entityClass).list ();
    }
    ...
    @SuppressWarnings("unchecked")
    public <T> T uniqueSql (final String sql, final Object ... paramList) {

        SQLQuery query = getCurrentSession ().createSQLQuery (sql);
        if (paramList != null) {
            setParameters (query, paramList);
        }
        return (T) query.addEntity (this.entityClass).uniqueResult ();
    }
