...
public class IndoorSpotToWeb {

	private static Logger logger = LoggerFactory
			.getLogger(IndoorSpotToWeb.class);
	...
	/** 分段 进入数据倒序(大->小) */
	public static List<List<Spot>> getPartList(List<Spot> listAll,
			int deviceType) {
		...
		for (Spot spot : listAll) {
			...
			if (last != null) {
				thisDistance = getIndoorDistance(spot, last);
			}

			// if (last == null
			// || Math.abs(spot.getReceive() - last.getReceive()) >
			// cutSecondLength
			// || !spot.getSystem().equalsIgnoreCase(last.getSystem())
			// || (Math.abs(spot.getReceive() - last.getReceive()) >
			// mincutSecondLength
			// && lastDistance > 3
			// && thisDistance > cutDistanceLength && thisDistance >
			// lastDistance * 6)) {

			if (last == null
					|| Math.abs(spot.getReceive() - last.getReceive()) > cutSecondLength
					|| !spot.getSystem().equalsIgnoreCase(last.getSystem())) {
	...
	public static double getDistance(Spot uploadPosition, Spot lastPosition) {
		double distanceAdd = DistanceUtil.distance(uploadPosition.getLng(),
				uploadPosition.getLat(), lastPosition.getLng(),
				lastPosition.getLat());// 里程
		return distanceAdd;
	}

	private static double getIndoorDistance(Spot uploadPosition,
			Spot lastPosition) {
		if ("1".equals(uploadPosition.getIndoor())
				&& "1".equals(lastPosition.getIndoor())
				&& uploadPosition.getPx() != null
				&& uploadPosition.getPy() != null
				&& lastPosition.getPx() != null && lastPosition.getPy() != null) {
			double distanceAdd = DistanceUtil.indoor_distance(
					uploadPosition.getPx(), uploadPosition.getPy(),
					lastPosition.getPx(), lastPosition.getPy());// 里程
			return NumUtil.round(distanceAdd, 1);
		}
		return getDistance(uploadPosition, lastPosition);
	}

	// --------------------------------------------------------------

	/** 里程等当天统计 */
	public static Report statisticToday(List<Spot> list, Report report,
			int deviceType) {
	...
	/** 里程等当天统计:速度段里程 */
	public static Report statisticTodaySpeed(List<Spot> list, Report report,
			Integer deviceType) {
		if (null == list || list.size() < 1)
			return report;
	...
	/** 速度百分比(速度分布) */
	public static List<SpeedVo> statisticSpeed(List<Report> list,
			Integer deviceType) {
		if (null == list || list.size() < 1)
			return null;
		...
		SpeedVo speedVo36 = new SpeedVo(speedPart[0] + "-" + speedPart[1]
				+ "km");
		SpeedVo speedVo61 = new SpeedVo(speedPart[1] + "-" + speedPart[2]
				+ "km");
	...
	public static int[] getByDeviceType(Integer deviceType) {
		if (deviceType.intValue() == 1)
			return car;
		if (deviceType.intValue() == 3)
			return person;
		if (deviceType.intValue() == 4)
			return car;
